...
public class BotAuthController(
    ...
    IOptions<Dictionary<string, AuthClient>> clients,
    IOptions<AuthConfig> config)
    : ControllerBase
{
    private readonly Dictionary<string, AuthClient> _clients = clients.Value;
    private readonly AuthConfig _config = config.Value;

    [HttpPost]
    public IActionResult Login([FromBody] TokenRequest auth)
    {
        if (_clients[auth.Name].ClientSecret != auth.ClientSecret) return Unauthorized();

        logger.LogInformation($"Logging in user {auth.Name}");
        var key = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(_config.SecretKey));
        var signingCreds = new SigningCredentials(key, SecurityAlgorithms.HmacSha256);

        var token = new JwtSecurityToken(_config.Issuer, $"{_config.Audience}/{auth.Name.ToLower()}",
